...
import { openAIChatCompletion } from "./http"
import {
	assertBulletArray,
	assertString,
	assertStringArray,
		...
		async pass3ReleaseNotes(input: EditorialPassInput): Promise<ReleaseNotesOutput> {
			const messages: ChatMessage[] = [
				{ role: "system", content: buildSystemPrompt() },
				{
					...
					content: [
						"Release notes pass.",
						"Task: write release notes for both non-developers and developers upgrading a package, in Markdown.",
						...
						"  - ### Highlights",
						"  - ### For Non-Developers (plain-language explanation of what changed and why it matters)",
						"  - ### Breaking Changes (include **Action required:** guidance)",
						"  - ### Upgrade Guide (Developers) (step-by-step upgrade instructions when actions are required)",
						"  - ### Fixes",
						"  - ### Deprecations",
						"  - ### Security",
						"  - ### Performance",
						...
						"- For Non-Developers: explain impact in plain language; avoid code, jargon, and internal implementation details.",
						"- If you include a Breaking Changes section, you MUST also include an Upgrade Guide (Developers) section.",
						"  - Upgrade Guide (Developers) must include concrete, step-by-step actions (e.g. version bumps, config changes, code changes).",
						"  - If evidence suggests a breaking change but no action is required, explicitly say **Action required:** No (and explain why).",
